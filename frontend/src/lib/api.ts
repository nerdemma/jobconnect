...
export async function postEmployee(
  employee: Employee & { walletAddress: string },
) {
...
export async function fetchEmployee(walletAddress: string) {
  return fetchJson<Employee>(
    `/api/employees?walletAddress=${encodeURIComponent(walletAddress)}`,
  );
}
...
export async function createJob(
  job: Omit<Job, "id" | "createdAt"> & { walletAddress: string },
) {
  return fetchJson<Job>("/api/jobs", {
    method: "POST",
